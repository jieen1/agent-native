...
<callout color="orange_bg">
	通过判据:F0–F10 全部完成 + §1.12 地基验收门全绿,才允许进入阶段二。每项五段:做什么 / 为什么是地基 / 现状证据 / 可证伪验收 / 依赖。现状统计(2026-07-16 复核):11 项全部已交付并部署 101;F2b 消费端切片(dispatcher 重试注入,T-F2-06)已在分支上交付,尚未部署 101。
</callout>
...
**执行前置(决定这套门能否真在 101 上跑)**:①**隔离靶位**——破坏性注入(空库迁移、删迁移、删 ACP 包重启、kill/重启进程)不得针对共享 `an-postgres` 生产库与 18 应用共用容器:空库迁移用 `createdb` 临时库,迁移号/建表注入在一次性 clone 或临时 project 上跑,进程注入在维护窗口对目标容器单独做;②**brain 单会话/低并发运行时,个人订阅可用**(2026-07-16 复核订正:既有教训的真实根因是「高频轮询账号端点」+「大量并发会话同时认证」两个具体模式,已分别经共享缓存修复与并发约束消解;单次/低并发验证 run 不复现,不必强制 API key,但仍需避免大量并发认证与重新引入高频轮询),否则依赖 brain/CC 在场的门(F7③、小单走查)不可稳定复现。**SSH 边界**:注入的**布置**(kill 进程、删包、推镜像)可用 SSH——它模拟 infra 失效;但每条的**观测判据**须页面/DB 可读。
