...
<callout color="orange_bg">
	通过判据:F0–F10 全部完成 + §1.12 地基验收门全绿,才允许进入阶段二。每项五段:做什么 / 为什么是地基 / 现状证据 / 可证伪验收 / 依赖。现状统计:缺失 8 项,部分 3 项。
</callout>

### F0 交付主干统一【缺失】(自举现状项,无直接设计章节;约束见 00 §8 A1)
...
### F2 执行器上下文管理(调研结论 B)【部分:机制在 core 已有,门控关闭】(设计出处 02 §4.1 C1–C4)
...
### F3 状态迁移守卫 + 派发不推进 + 人工流转通道【缺失】(设计出处 02 §8 守卫表)
...
### F4 能力面矩阵(评审只读)+ spec/评审分离【缺失】(设计出处 02 §5.4 / §3 评审独立性)
...
### F5 任务拆分阈值(规划前置契约)【缺失】(设计出处 02 §3.10 拆分契约)
...
### F6 迁移对账 + 迁移冒烟 + 迁移号防撞【缺失】(设计出处 02 §7 迁移冒烟 / §8 标识权威 / 03 §2 核对清单)
...
### F7 遥测与身份单一事实源【部分】(设计出处 04 §7 / §13 / §10 / §6)
...
### F8 回链完整性 + itemKey 分配权威【缺失】(设计出处 02 §8 / 03 §11 序列器)
...
### F9 确定性回写通道(M4-D)【缺失】(设计出处 02 §6 回写通道 / 04 §13 tracker-client)
...
- **S5 评审卡:按 nature 装配的结构化核对清单控件**(F6)——评审卡已加,核对项控件未做。
