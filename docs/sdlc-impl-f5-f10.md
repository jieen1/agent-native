...
| `actions/create-work-item.ts`                  | itemKey 分配改**序列器单点**:Postgres `UPDATE tracker_project_seq SET next_seq=next_seq+1 WHERE project_id=$1 RETURNING next_seq`(原子);行不存在→先 INSERT(on conflict do nothing)再 UPDATE。删除现状「count 现有条数」逻辑(SDLC-038 重号根因)。**历史重号不改**(只加不改):现存 SDLC-032~036 双号保持,UI 消歧见 4B。                                                                                                                                                                                                                                                                                                 |
