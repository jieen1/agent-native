...
### F0 交付主干统一【缺失】(自举现状项,无直接设计章节;约束见 00 §8 A1 单机部署假设)
...
### F4 能力面矩阵(评审只读)+ spec/评审分离【缺失】(设计出处 02 §5.4 能力面矩阵 / §3 评审独立性)
...
### F7 遥测与身份单一事实源【部分】(设计出处 04 §7 模型注册表 / §13 采集契约 / §10 遥测可信卡 / §6 turn 终态判定)
...
### F8 回链完整性 + itemKey 分配权威【缺失】(设计出处 02 §8 回链载荷+标识权威 / 03 §11 序列器)
...
### F10 引擎终态传导完备【部分】(设计出处 02 §4 R9 终态传导不变量)
