...
- **页面**:涉及——S4 工作项详情(受守卫流转对话框,含未派发项关闭);
  原型:**已完成**(2026-07-16 复核订正:主干 c0c7e66d1 起即含完整对话框本体,
  c18c8ee84 又补 fm-overlay 开合动效;原"对话框本体未做"表述过期)。
...
- **S4 受守卫流转对话框**(F3)——已补(2026-07-16 复核订正:主干 c0c7e66d1 起即含
  完整对话框本体——受限目标状态选择/强制 reason/按状态证据字段,c18c8ee84 补
  fm-overlay 开合动效;生产 design s4 与 repo md5 一致);其完整交互由独立的
  **F1–F4 实施细则文档**承接。
...
进程类注入在维护窗口对目标容器单独做;②**brain 单会话/低并发运行时,个人订阅可用**(2026-07-16
复核订正:既有教训的真实根因是「高频轮询账号端点」+「大量并发会话同时认证」两个具体模式,已分别
经 `account-usage.ts` 共享缓存修复与"不要同时起几十个并发 brain/CC-worker 会话"约束消解;单次/
低并发的自动化验证 run 不复现这两个模式,不必强制 API key)。仍需避免:大量并发会话同时认证、
重新引入高频轮询账号端点。否则依赖 brain/CC 在场的门(F7③、小单走查)不可稳定复现。
