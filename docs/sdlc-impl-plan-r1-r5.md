...
上游文档对"受守卫流转对话框"的现状表述已过期,且涉及**两个不同对象**(2026-07-16
独立审查订正出处标注:原文此处误把《路线图》§1.11a 写作《对齐审查》§1.11a):
《路线图》§1.11a 称原型屏"S4 受守卫流转对话框(F3)——shield-lock 入口已加,对话框
本体未做";《对齐审查》§二/§四 R3 则称生产 UI"守卫流转对话框后端在、UI 未接线"。
**本次执行级展开前逐文件核实(`git blame`/`git log -S` 定位),两个说法均已过期**——
生产 UI 一侧证据如下(原型一侧:`prototypes/s4-work-item.html` 自 `c0c7e66d1`,2026-07-12
起即含完整对话框本体,`c18c8ee84` 又补 fm-overlay 开合动效):
...
R3 的待办工作量;建议本文发布后另开一次对《路线图》§1.11a 与《对齐审查》§二/§三/§四 R3
相应措辞的小订正(不改判据,只更新状态描述;03 章两副本经核实并不含"未做"现状表述,
无需订正)。
...
- **依赖**:WP-R1.1 建议先行(降低卡死 bug 干扰验证判断的风险);对齐审查原文的硬门槛是
  ...
  WP-R1.1 先行。
- **验收判据**(继承《路线图》§1.12 "执行前置"两条;2026-07-16 独立审查订正出处——
  §1.12 在《路线图》,《对齐审查》无此节):①隔离靶位——不得针对共享
...
> (继承其 §二"免费收敛机会"段)、Foundry v3 动效在生产界面落地(继承其 §五"落地"段)。
> **验收总纲**:一个人不碰 agent 聊天窗,纯 UI 走完场景①的人工环节。
...
- **保留判据**(供未来回归核对,不是新开发用):路线图 F3 可证伪验收①②③(直接写
  ...
  《路线图》§1.12 地基验收门第 3 项执行(出处订正同 WP-R1.2),不因 UI 已在场而免验。
- **依赖**:无(已完成)。
...
- **量级**:2–3 天(新页面+两个新 action+导航接线;评审卡"按 nature 装配的结构化核对清单
  控件"这一 03§2 设计、《路线图》§1.11a(S5)记录在案的欠账一并在此做)。
...
- **依赖**:无硬依赖(orchestrator `RunView`/`WorkspaceView` 已存在,只是没有从 tracker
  深链过去;`list-audit-events` 是框架既有 action,只缺渲染);最终验收与 R2 走查暴露的
  实际证据需求联动(见 §7.1)。
...
- **验收判据**(继承 F7 可证伪验收③、04§10 全文;2026-07-16 订正——"健康页红灯"
  是 F7③,F7④ 是 turn 终态判定):声明开启的能力初始化失败必须在本页显式红灯
...
- **目标**:把 02§3.1 定义的
  `dev→qa→reviewer→gatekeeper→diff-audit→commit+PR→ci-watch→merge-pr` 流水线
  (02§3.1 含 workspace 起步共九节点)从"仓内零种子"变成入库、带版本、种子化的真实模板。
- **继承的现状表述订正**(2026-07-16 独立审查,101 DB 只读实测):《对齐审查》"仅单节点
  sdlc-dev 活在 101 DB"一句不确——live `v3_workflow_templates` 实有自举遗留的
  `sdlc-issue-pipeline`(14 节点,v1–v4)、`sdlc-review`(v1–v5)、`sdlc-audit`、
  `sdlc-full`、`sdlc-promote`、`sdlc-dev`(v1–v5)多套模板;"仓内零种子"仍成立。
  故本 WP 与路线图 §3 口径一致:把 DB 中的自举遗留模板**收编入仓、硬化、种子化**,
  不是从零新建。
...
- **验收判据**(继承 02§3.10 工作流选择器规则;2026-07-16 订正——工作流选择器在
  02 章,03 章无 §3.10):缺陷/生产问题→`hotfix`、文档→
...
<td>清理 `sdk-brain-session.ts` 残余硬编码(2026-07-16 核实精确化:vLLM endpoint/model/key 已读 env,但 env 缺省时**静默回退**到硬编码默认值 `192.168.1.250:9000`/`sk-vllm-local`(:22-27),应去回退、缺配置 fail loud;`MCP_URL` 的 `localhost:3002`(:20)为纯硬编码,应改走 env)</td>
...
<td>R3.4 建议在 R3.2 之后;R3.3 的最终验收需要 R2 走查暴露的实际证据需求;R3.8 的 token 接入可先行,但其"两应用统一动效语言"完整验收依赖 R3.2/R3.5/R3.6 新页面就绪(新页面是招牌模式的主要落点)</td>
</tr>
<tr>
<td>R4 三个 WP</td>
...
<td>WP-R4.1(`action` 确定性节点类型与种子机制,WP-R5.1 复用)、WP-R4.3(ui-spec 技能)、F8(已交付);WP-R5.2 依赖 WP-R5.1 的发布管道</td>
</tr>
<tr>
<td>§6 工程卫生七项</td>
