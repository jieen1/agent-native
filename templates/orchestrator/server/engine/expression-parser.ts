...
function tokenize(input: string): Tok[] {
  ...
  while (i < len) {
    ...
    if (c === 34 || c === 39) {
      ...
      out.push({
        type: TK.STRING,
        value: input.slice(i + 1, j).replace(/\\(.)/g, "$1"),
      });
      i = j + 1;
      continue;
    }
    ...
    if (c === 46) {
      if (
        i + 1 < len &&
        input.charCodeAt(i + 1) >= 48 &&
        input.charCodeAt(i + 1) <= 57
      ) {
        ...
        if (i === 0 || !isIdentOrDot(input.charCodeAt(i - 1))) {
          ...
          while (
            j < len &&
            input.charCodeAt(j) >= 48 &&
            input.charCodeAt(j) <= 57
          )
            j++;
    ...
    if (c >= 48 && c <= 57) {
      let j = i;
      while (j < len && input.charCodeAt(j) >= 48 && input.charCodeAt(j) <= 57)
        j++;
      // handle decimal
      if (j < len && input.charCodeAt(j) === 46) {
        ...
        if (
          j + 1 < len &&
          input.charCodeAt(j + 1) >= 48 &&
          input.charCodeAt(j + 1) <= 57
        ) {
          j++;
          while (
            j < len &&
    ...
    if (c === 40) {
      out.push({ type: TK.LPAREN });
      i++;
      continue;
    }
    if (c === 41) {
      out.push({ type: TK.RPAREN });
      i++;
      continue;
    }
    if (c === 91) {
      out.push({ type: TK.LBRACKET });
      i++;
      continue;
    }
    if (c === 93) {
      out.push({ type: TK.RBRACKET });
      i++;
      continue;
    }
    if (c === 44) {
      out.push({ type: TK.COMMA });
      i++;
      continue;
    }
    if (c === 45) {
    ...
    if (isIdentStart(c)) {
      ...
      if (word === "true") {
        out.push({ type: TK.BOOL, value: true });
      } else if (word === "false") {
        out.push({ type: TK.BOOL, value: false });
      } else if (word === "null") {
        out.push({ type: TK.NULL });
      } else {
        out.push({ type: TK.IDENT, value: word });
      }
      i = j;
      continue;
    }
...
function peek(p: P): Tok {
  return p.toks[p.pos];
}

function advance(p: P): Tok {
  return p.toks[p.pos++];
}
...
function expect(p: P, type: TK, value?: string | number): Tok {
  ...
  if (t.type !== type || (value !== undefined && t.value !== value)) {
    throw new Error(
      `Expected ${type}${value !== undefined ? "(" + value + ")" : ""}, got ${t.type}`,
    );
  }
  return t;
}
...
function doCmp(op: string, l: unknown, r: unknown): boolean {
  ...
  switch (op) {
    ...
    case "!=":
      return ln !== rn && ls !== rs;
    case ">":
      return Number.isNaN(ln) ? ls > rs : ln > rn;
    case ">=":
      return Number.isNaN(ln) ? ls >= rs : ln >= rn;
    case "<":
      return Number.isNaN(ln) ? ls < rs : ln < rn;
...
function unaryE(p: P): unknown {
  ...
  if (top.type === TK.OP && top.value === "-") {
    ...
    const v = unaryE(p);
    if (typeof v !== "number")
      throw new Error("Unary minus requires a numeric operand");
    return -v;
  }
  return prim(p);
}
...
function callFn(p: P): unknown {
  ...
  if (peek(p).type !== TK.RPAREN) {
    do {
      args.push(expr(p));
    } while (peek(p).type === TK.COMMA && (advance(p), true));
  }
  expect(p, TK.RPAREN);
...
export function evaluateExpression(
  ...
  if (peek(parser).type !== TK.EOF) {
    throw new Error(
      `Unexpected token after expression at position ${parser.pos}`,
    );
  }
  return result;
}
