...
// ── F7 usage-capture at NodeRunner.run() return-value layer (T-F7-03/04/05/06) ──
//
// R3-corrected injection point: NodeRunner is already a full mock class in
// this file (`hoisted.MockNodeRunner`), so the fake `RuntimeExecutor` passed
// to `new V3Dispatcher(...)` never actually runs — spying on
// `MockNodeRunner.prototype.run` IS "injecting the executor.run() return
// value" one layer up, exactly what these tests exercise. Assertions read
// camelCase in-memory mock fields (`mockDb.spawns[0].tokensInput`), never a
// live DB column name.
