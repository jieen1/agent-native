...
describe("renderTemplate", () => {
  ...
  it("interpolates deps path", () => {
    expect(renderTemplate("Score: {{ deps.research.output.score }}", ctx)).toBe(
      "Score: 95",
    );
  });

  it("serializes arrays as JSON", () => {
    expect(renderTemplate("{{ inputs.tags }}", ctx)).toBe(
  ...
  it("passes through plain text", () => {
    expect(renderTemplate("No interpolation here", ctx)).toBe(
  ...
  it("handles multiple interpolations", () => {
    expect(
      renderTemplate("A={{ inputs.name }} B={{ inputs.count }}", ctx),
    ).toBe("A=Alice B=5");
  });
});
