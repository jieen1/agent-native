...
const DEV = "vllm";
// Standing policy: DAG worker nodes never use claude-code — every worker
// (dev, review, merge-review, escalation) routes through vllm's active
// runtime_configs row (currently Aliyun qwen3.8-max-preview). claude-code is
// reserved for the brain's own MCP-tool-calling session, never a DAG node.
const REVIEW = "vllm";
      ...
      nodes: [
        {
          ...
          deps: [],
          prompt:
            '你是发布工程师,执行 Phase G 晋升:把 sprint 分支以 merge-commit 合入 base 分支,push 后删除远程 sprint 分支。用 Bash 在当前 workspace 的 git 仓库操作:\n1. sprintBranch = {{inputs.sprintBranch}} ; baseBranch = {{inputs.baseBranch}}\n2. git fetch origin --prune(prune 让已在远端删除的分支在本地也不再可见,这是后续识别"已完成晋升"的关键)。\n3. 幂等检查 1(分支存在性):若 `origin/{{inputs.sprintBranch}}` 在 fetch --prune 之后已不存在(说明上一次晋升已完成并清理了分支),报告 already-promoted(skip 原因:分支不存在,视为已完成),不报错,直接结束,不做任何操作。\n4. 幂等检查 2(领先提交数):若分支存在,COMMITS_AHEAD=$(git rev-list --count origin/{{inputs.baseBranch}}..origin/{{inputs.sprintBranch}})。若为 0,报告 already-promoted(skip 原因:无领先提交),直接结束,不删除分支。\n5. 若 COMMITS_AHEAD > 0:git checkout {{inputs.baseBranch}} && git merge --no-ff origin/{{inputs.sprintBranch}} -m "Promote sprint {{inputs.sprintBranch}} into {{inputs.baseBranch}}"(保留 sprint 边界的 merge-commit)。若有冲突,不要自动解决,报告冲突并停止(顺序合并红线:永不自动解冲突,不 force push)。\n6. 合并成功后 git push origin {{inputs.baseBranch}}。\n7. push 成功后 git push origin --delete {{inputs.sprintBranch}} 删除远程 sprint 分支(合并边界已保留在 merge-commit 里,不再需要保留分支本体)。若删除失败(如权限不足),报告删除失败但不回滚已完成的合并。\n8. 报告:COMMITS_AHEAD 数、是否晋升、merge-commit sha 或 skip 原因(区分"分支不存在"和"无领先提交"两种 skip)、是否有冲突、sprint 分支是否已删除。绝不 force push,绝不自动解冲突。',
          workspace: "{{inputs.workspaceId}}",
        },
      ],
    },
