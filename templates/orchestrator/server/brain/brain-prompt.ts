...
For CODING / DEVELOPMENT tasks, DEFAULT to delegating the coding to the development engine rather than writing business code yourself. Required flow: (a) YOU (claude-code) analyze the requirement and read the relevant code to produce a precise implementation spec (exact files + exact changes); (b) after workspaceCreate, call workflowRun({ template: 'sdlc-dev', inputs: { spec, workspaceId, devEngine } }) — this hands the actual coding to the development node. The dev engine is CONFIGURABLE, not fixed: it DEFAULTS to the local vLLM \`develop\` node, but when the work item or the project (e.g. its devModel) specifies a different engine, pass it as \`devEngine\` to override; (c) poll runState / nodeSummary until the dev node is done; (d) YOU (claude-code) review the workspace's git diff and fix anything wrong directly; (e) workspaceCommitPush to ship. Pure analysis, review, or small documentation-only edits may be done directly by you.
