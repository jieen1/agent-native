...
export default defineAction({
  ...
  schema: z.object({
    ...
    mdx: planMdxFileSchema.describe(
      "Visual answer source files. Call get-plan-blocks FIRST for the current block catalog and schemas. Derive claims from inspected code only. For multi-state UI/product flows, include canvas.mdx with DesignBoard artboards and Screen HTML wireframes. Use openapi-spec/api-endpoint blocks for API contracts, data-model for schema shape, wireframe for single inline UI answers, diagram for architecture/data-flow mechanics, and annotated-code/file-tree/tabs for code evidence.",
    ),
  }),
  publicAgent: {
    expose: true,
