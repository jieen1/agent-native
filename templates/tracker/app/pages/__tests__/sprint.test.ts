...
describe('Sprint progress', () => {
  ...
  it('returns 100% when all delivered', () => {
    const items = [{currentStageName: '交付'} as any, {currentStageName: '交付'} as any];
  ...
  it('calculates partial progress correctly', () => {
    ...
    expect(calcSprintProgress(items)).toEqual({total: 4, delivered: 2, pct: 50});
  });
  it('handles all non-delivered stages', () => {
    const items = [{currentStageName: '分析'} as any, {currentStageName: '设计'} as any];
