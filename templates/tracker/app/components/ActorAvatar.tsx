 ...
 * does in foundry-components.html.
 */
...
export function ActorAvatar({
  ...
  return (
    <span
      ...
      className={cn(
        "tk-avatar",
        kind === "agent" && "tk-avatar--agent",
        kind === "brain" && "tk-avatar--brain",
        className,
      )}
      style={
        size
    ...
    >
      ...
      {live ? <span className="tk-avatar-presence" aria-hidden="true" /> : null}
    </span>
  );
}
...
export function ActorAvatarStack({
  ...
  return (
    <span className={cn("tk-avstack", className)}>
      {shown.map((a) => (
