...
describe("<StatusIcon>", () => {
  it.each(ALL_TONES)(
    ...
    (tone) => {
      ...
      expect(icon?.className).toContain("tk-status-icon");
      expect(icon?.className).toContain(`tk-status-icon--${tone}`);
  ...
  it("defaults to md size (no extra size class)", () => {
    ...
    const icon = container.querySelector("span");
    expect(icon?.className).not.toContain("tk-status-icon--sm");
    expect(icon?.className).not.toContain("tk-status-icon--lg");
  });

  it("applies sm/lg size classes when requested", () => {
    const { container: sm } = render(<StatusIcon tone="ok" size="sm" />);
    expect(sm.querySelector("span")?.className).toContain("tk-status-icon--sm");

    const { container: lg } = render(<StatusIcon tone="ok" size="lg" />);
    expect(lg.querySelector("span")?.className).toContain("tk-status-icon--lg");
  });
