...
// M5 度量复盘 — Sprint 发布 (publish) step.
//
 ...
 *  keyed on sprint id so a second call never writes a duplicate file. */
function writeReleaseChangelogEntry(sprintId: string, sprintName: string): void {
  ...
  // Idempotency: if the file already exists (a previous release wrote it), skip.
...
export default defineAction({
  ...
  run: async (args) => {
    ...
    // ── Idempotency guard (authoritative — safe on its own) ────────────────
    ...
    // ── Changelog: write a pending entry (best-effort, never blocks release) ─
    let changelogWritten = false;
    try {
      writeReleaseChangelogEntry(sprint.id, sprint.name);
      changelogWritten = true;
    } catch {
      // A changelog write failure (e.g. read-only fs) must not roll back the
      // release itself; the entry can be added manually later.
    }
