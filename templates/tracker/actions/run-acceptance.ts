...
export default defineAction({
  ...
  run: async (args, ctx) => {
        ...
        if (!sc.action || !/^[a-z0-9-]+$/.test(sc.action)) {
          ...
          try {
            ...
            if (!entry) {
              ...
              actual = (await entry.run(sc.args ?? {}, ctx)) as Record<
                string,
                unknown
              >;
            }
    ...
    // Forward `ctx` so trigger-stage / complete-stage attribute their OWN
    // activity rows to whoever actually called run-acceptance (agent tool
    // call vs the UI's "Run acceptance" button), not to a default.
      ...
      await createArtifactAction.default.run(
        {
          ...
          // The evidence report itself is always machine-generated by this
          // scenario runner, regardless of which surface triggered the run —
          // keep this explicit rather than deriving from ctx.
    ...
    // Who ran the acceptance check (agent tool call vs. the UI's "Run
    // acceptance" button, which gives the UI parity with the agent path) —
    // not the fact that the check itself is automated.
