...
const baseDep = (overrides: Partial<DependencyStatusInput>): DependencyStatusInput => ({
  id: 'dep_001',
  itemKey: 'PROJ-1',
  status: 'running',
  currentStageName: '实施',
...
describe('isGateCleared', () => {
  ...
  it('status=open, stage=待办 → not cleared', () => {
    expect(isGateCleared(baseDep({ status: 'open', currentStageName: '待办' }))).toBe(false);
  });
...
describe('evaluateDispatchGate', () => {
  ...
  it('single dependency done AND has a branch → chainedBranch returns that branch (acceptance scenario: B blocked-by A only, A done)', () => {
    const deps = [
      baseDep({ itemKey: 'PROJ-1', status: 'done', branch: 'orchestrator/run-a-impl' }),
    ];
    const result = evaluateDispatchGate(deps);
    expect(result.ready).toBe(true);
    expect(result.blockedBy).toEqual([]);
  ...
  it('all deps done → chainedBranch is null (no branch chain with multiple cleared deps)', () => {
    const deps = [
      baseDep({ id: 'd1', itemKey: 'PROJ-1', status: 'done', branch: 'feature/a' }),
      baseDep({ id: 'd2', itemKey: 'PROJ-2', status: 'done', branch: 'feature/b' }),
    ];
    const result = evaluateDispatchGate(deps);
    expect(result.ready).toBe(true);
    expect(result.blockedBy).toEqual([]);
  ...
  it('blockedBy is sorted by itemKey ascending', () => {
    ...
    const result = evaluateDispatchGate(deps);
    expect(result.blockedBy.map((d) => d.itemKey)).toEqual(['PROJ-1', 'PROJ-3', 'PROJ-9']);
  });
});