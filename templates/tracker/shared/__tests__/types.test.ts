...
describe('STAGE_ORDER', () => {
  ...
  it('every element is assignable to the StageName type (compile-time check at runtime)', () => {
    for (const s of STAGE_ORDER) {
      ...
      expect(['待办','分析','设计','实施','测试','验收','交付'].includes(s)).toBe(true);
    }
  });
});
...
describe('StageName type values', () => {
  const valid: StageName[] = ['待办','分析','设计','实施','测试','验收','交付'];
...
describe('WorkItemStatus values', () => {
  it('has the six work item states', () => {
    ...
    expect(valid).toEqual(['open','queued','running','dispatched','done','failed']);
...
describe('WorkItem-shaped object validation', () => {
  it('a fully-valid item passes all field validators', () => {
    ...
    expect(STAGE_ORDER.includes(validItem.currentStageName as StageName)).toBe(true);
    expect(['low', 'medium', 'high'].includes(validItem.risk)).toBe(true);
    expect(['manual', 'auto'].includes(validItem.executionMode)).toBe(true);
    expect(Number.isInteger(validItem.priority) && validItem.priority >= 0).toBe(true);
    expect(validItem.plannedStages.every((s) => STAGE_ORDER.includes(s as StageName))).toBe(true);
  });
